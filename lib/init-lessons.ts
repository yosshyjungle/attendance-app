...
export async function ensureLessonsForDate(
  date: string,
  classId: string
): Promise<{ id: string; period: number; name: string }[]> {
  ...
  if (existing.length > 0) {
    return existing.map((l) => ({ id: l.id, period: l.period, name: l.name }));
  }
  ...
  for (const slot of toCreate) {
    ...
    const lesson = await prisma.lesson.create({
      data: {
        ...
        startTime: start,
        endTime: end,
        qrCodeToken: `lesson-${randomBytes(16).toString("hex")}`,
      },
    });
    created.push(lesson);
  }

  return created.map((l) => ({ id: l.id, period: l.period, name: l.name }));
}
